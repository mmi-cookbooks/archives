...
class Chef::Resource
  class UntarArchive < Chef::Resource::File
    ...
    def nice(arg = nil)
      arg = arg.is_a?(Numeric) ? arg.to_i : arg
      set_or_return(
        :nice,
        arg,
        callbacks: {
          'not in valid numeric range. must be an integer between -19 and 19' => lambda do |m|
            if m.nil?
              true
            else
              m.is_a?(Integer) && m >= -19 && m <= 19
            end
          end
        }
      )
...
class Chef::Provider
  class UntarArchive < Chef::Provider::File
    def action_create
      fail 'archive missing!' unless ::File.exist? @new_resource.path
      ...
      # extract block
      if !::File.exist?(createsPath) || ::File.mtime(@new_resource.path) > ::File.mtime(createsPath)
        # TODO: ensure container path exists!
        # delete block
        to_remove = "#{new_resource.container_path}/#{new_resource.delete_dir_in_container}"
        if !new_resource.delete_dir_in_container.nil? && ::File.exist?(to_remove)
